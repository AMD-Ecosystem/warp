...
// invisible there and calls from kernels would fail to compile. A single
// host+device function with an internal device/host branch compiles cleanly
// under both nvcc and hipRTC.
CUDA_CALLABLE inline wp::BVHPackedNodeHalf bvh_load_node(const wp::BVHPackedNodeHalf* nodes, int index)
{
    ...
    // On NVIDIA __ldg routes through the read-only / texture cache. On AMD (HIP)
    // __ldg maps to a plain load since CDNA/RDNA have no separate read-only cache;
    // the float4 cast still guarantees a single 128-bit global-memory transaction
    // per node, matching the L2 cache-line granularity on MI250X / MI300X.
